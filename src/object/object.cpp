...
void object_moviment::auto_move(object_rectangle_attributes& frame,
  ...
  if (frame.rectangle.x < 0 ||
      frame.rectangle.x + frame.rectangle.w > screen.width) {
    frame.speed_x = -frame.speed_x;
  }

  if (frame.rectangle.y < 0 ||
      frame.rectangle.y + frame.rectangle.h > screen.height) {
...
void object_moviment::random_position_x(object_rectangle_attributes& frame,
                                        core::core_screen& screen) {
  int n_position =
      std::experimental::randint(10, (screen.width - frame.rectangle.x));
...
void object_moviment::bellow_move(object_rectangle_attributes& frame,
  ...
  if (frame.rectangle.y > screen.height ||
      frame.rectangle.y == screen.height - frame.rectangle.h) {
    ++object_leaks;
